		...
		while(fgets(line, sizeof(line), fp)){
			...
			// uv/
				...
				if(sscanf_s(line + 2, "%f %f %f", &x, &y, &z) == 3){
					vertices[vcnt++] = (vec3){ x,y,-z };//blenderはなんか右手系を維持しようとするからzを反転する/
					//blenderの設定は上と奥の方向しか決められない　じゃあ右はどうなるのかというと右手法を維持するように勝手に決められる/
					//だから初期設定は　奥:z- 上:y+ の場合右はx+になる/
					//そして、　奥:z+ 上:y+ の時、右はx-になる/
					//だけど今回やってほしいのは　奥:z+ 上:y+ 右:x+ だからこれは右手法を維持するblenderでは設定負荷(多分)/
					//だからこっち側で調整するしかない(と思う)/
					//blender上ではデフォルトの　奥:z- 上:y+ 右:x+ の状態で出力して読み込み側でzを反転することでこれを解消..しようとしてる/
					//まあ成功してそうやし大丈夫そうかな　ちなみにこれに気付いたのはテクスチャのある程度の位置があってるのに/
					//なぜか左右反転してたからよく見てみるとモデル自体が左右反転しててそこで気づいた 恐ろしい/
			...
			// 先頭がf/
				...
				//右手系を左手系として読み込むからzが反転　その影響で右回りと左回りが変わる　例えば、/
				//x方向を向いてる壁は例えばx+が表ならzが反転してもx+のまま　なのに頂点はz反転の影響で面が反転/
				//法線は頂点座標からのヘントの外積だから法線も反転する　だからインデックスる配列を反転する必要がある/
				//y方向を向いてる壁も同様　y+はy+のまま　なのに反転する　だからインデックス配列の反転が必要/
				//z方向はz+はz-になる必要がある　だけどzが反転してもz+はz+のまま(数学的にはわからんけど感覚的にイメージすればわかるはず)/
				//だからこれもインデックス配列を反転する必要がある/
				int parsed = sscanf_s(line, "f %d/%d%*[^ ] %d/%d%*[^ ] %d/%d", &index[2], &uvIndex[2], &index[1], &uvIndex[1], &index[0], &uvIndex[0]);
